...
public class LandingPageController {
    ...
    public String post (@RequestParam("title") String title,  @RequestParam("newPage")int newPage, Model model) {

        AllBooks searchedBook = allBookRepository.findByTitle(title); //find book
        if (searchedBook.getPagesTotal() >= newPage) {
            searchedBook.setPageCurrent(newPage);
            searchedBook.setProgress();//set new page
        }

        allBookRepository.save(searchedBook);
        List<Quotes> quotesFromBook = quoteRepository.findAllQuotesByBookId(searchedBook.getId());
        model.addAttribute("BOOK", searchedBook);
        model.addAttribute("QUOTES", quotesFromBook);
